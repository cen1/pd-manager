...
string UTIL_SSRead( stringstream &SS, uint32_t start, uint32_t size )
{
	...
	SS.seekg( start, ios::cur );
	SS.readsome( Buffer, size );

	string Str = string( Buffer, size );
	delete Buffer;
...
uint32_t UTIL_GetMostFrequentValue(map<unsigned char, unsigned char> * map) {

	std::map<uint32_t, uint32_t> counter;
	for (auto const& i : *map) {
		counter[i.second]++;
	}
	uint32_t valueWithMaxOccurence = std::max_element(
		std::begin(counter), std::end(counter), [](auto p1, auto p2) {
...
uint32_t UTIL_GetMostFrequentValue(map<unsigned char, uint32_t> * map) {
...
void CONSOLE_Print(CGamePlayer *reporter, string message) {
	CONSOLE_Print("[" + reporter->GetName() + "]" + message);
}
