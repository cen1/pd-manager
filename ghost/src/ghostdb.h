...
public:
	CCallablePlayerCheck( string nName, string nServer ) : CBaseCallable( ), m_Name( nName ), m_Server( nServer ), m_Result( NULL ) { }
...
public:
	CDBPlayer( CDBGPS *nGPS, CDBDiv1DPS *nDiv1DPS, string nServer, string nName, string nFrom, string nLongFrom, uint32_t nAccessLevel, bool nBanned )
		: m_GPS( nGPS ), m_Div1DPS( nDiv1DPS ), m_Server( nServer ), m_Name( nName ), m_From( nFrom ), m_LongFrom( nLongFrom ), m_AccessLevel( nAccessLevel ), m_Banned( nBanned ) { }
	~CDBPlayer( );
...
public:
	CDBGamePlayer( string nName, string nIP, uint32_t nSpoofed, string nSpoofedRealm, uint32_t nReserved, uint32_t nLoadingTime, uint32_t nLeft, string nLeftReason, uint32_t nTeam, uint32_t nColour, bool nGProxy )
		: m_Name( nName ), m_IP( nIP ), m_Spoofed( nSpoofed ), m_SpoofedRealm( nSpoofedRealm ), m_Reserved( nReserved ), m_LoadingTime( nLoadingTime ), m_Left( nLeft ), m_LeftReason( nLeftReason ), m_Team( nTeam ), m_Colour( nColour ), m_GProxy( nGProxy ), m_Banned( false ) { }
	~CDBGamePlayer( ) { }
...
public:
	CDBGPS( uint32_t nTotalGames ) : m_TotalGames( nTotalGames ) { }
	~CDBGPS( ) { }
