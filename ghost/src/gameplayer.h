...
private:
	...
	bool m_SpoofedNoticeSent;					// if the spoof check notice was sent in the lobby
	...
	bool m_FingerSent;							// if we've sent a battle.net /finger for this player yet (for mute checking)
	...
	bool m_selectedForGameLog = true;			// whether game log will be written from this player. Prevents log duplication in maps that send events from all players, not just the host (like d1stats DotA).
	queue<BYTEARRAY> m_GProxyBuffer;
	uint32_t m_GProxyReconnectKey;
	uint32_t m_LastGProxyAckTime;

	CDBPlayer *m_DBPlayer;
	bool m_RemakeVote;
	BYTEARRAY m_IgnoredPIDs;
	
	uint32_t m_XPAMGProxyVersion;
	bool m_gpsW3VersionSent = false;

public:
	uint32_t GetXPAMGProxyVersion( )			{ return m_XPAMGProxyVersion; }
	bool GetGpsW3VersionSent() { return m_gpsW3VersionSent; }

	BYTEARRAY GetIgnoredPIDs( )					{ return m_IgnoredPIDs; }
	CDBGPS *GetGPS( );
	CDBDiv1DPS *GetDiv1DPS( );
	string GetFrom( );
	string GetLongFrom( );
	uint32_t GetAccessLevel( );
	bool GetIsBanned( );
	bool GetIsAdmin( );
	bool GetRemakeVote( )						{ return m_RemakeVote; }

	void SetFrom( string nFrom );
	void SetLongFrom( string nLongFrom );
	void SetIgnorePID( unsigned char PID );
	bool SetUnignorePID( unsigned char PID );
	void SetRemakeVote( bool nRemakeVote )		{ m_RemakeVote = nRemakeVote; }
...
public:
	...
	CGamePlayer( CDBPlayer *nDBPlayer, CPotentialPlayer *potential, unsigned char nPID, string nJoinedRealm, string nName, BYTEARRAY nInternalIP, bool nReserved );
	virtual ~CGamePlayer( );
	...
	bool GetSpoofedNoticeSent( )				{ return m_SpoofedNoticeSent; }
	...
	bool GetFingerSent()						{ return m_FingerSent; }
	...
	bool GetSelectedForGameLog()				{ return m_selectedForGameLog;  }
	...
	void SetSpoofedNoticeSent( bool nSpoofedNoticeSent )							{ m_SpoofedNoticeSent = nSpoofedNoticeSent; }
	...
	void SetGpsW3VersionSent(bool nGpsW3VersionSent)								{ m_gpsW3VersionSent = nGpsW3VersionSent;  }
	void SetSelectedForGameLog(bool nSelectedForGameLog)							{ m_selectedForGameLog = nSelectedForGameLog; }
