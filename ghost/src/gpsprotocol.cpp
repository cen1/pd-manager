...
BYTEARRAY CGPSProtocol :: SEND_GPSC_INIT( uint32_t version )
{
	...
	CONSOLE_Print("[GHOST] Sent GPS_INIT");

	return packet;
}

BYTEARRAY CGPSProtocol::SEND_GPSS_W3VERSION(W3Version * w3version)
{
	BYTEARRAY packet;
	packet.push_back(GPS_HEADER_CONSTANT);
	packet.push_back(GPS_W3VERSION);
	packet.push_back(0);
	packet.push_back(0);

	//16bit unsigned ints
	UTIL_AppendByteArray(packet, w3version->m_major, false);
	UTIL_AppendByteArray(packet, w3version->m_minor, false);
	UTIL_AppendByteArray(packet, w3version->m_patch, false);
	UTIL_AppendByteArray(packet, w3version->m_build, false);
	AssignLength(packet);

	CONSOLE_Print("[GHOST] Sent GPS_W3VERSION");
