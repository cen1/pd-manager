...
CGamePlayer :: CGamePlayer( CGameProtocol *nProtocol, CBaseGame *nGame, CTCPSocket *nSocket, unsigned char nPID, string nJoinedRealm, string nName, BYTEARRAY nInternalIP, bool nReserved ) : m_DBPlayer( NULL ), CPotentialPlayer( nProtocol, nGame, nSocket ),
...
m_LoggedIn( false ), m_Spoofed( false ), m_SpoofedNoticeSent( false ), m_Reserved( nReserved ), m_WhoisShouldBeSent( false ), m_WhoisSent( false ), m_FingerSent(false), m_DownloadAllowed( false ), m_DownloadStarted( false ), m_DownloadFinished( false ), m_FinishedLoading( false ), m_Lagging( false ),
m_DropVote( false ), m_KickVote( false ), m_Muted( false ), m_LeftMessageSent( false ), m_GProxy( false ), m_GProxyDisconnectNoticeSent( false ), m_GProxyReconnectKey( rand() ), m_LastGProxyAckTime( 0 ), m_XPAMGProxyVersion( 0 )
{

}

CGamePlayer :: CGamePlayer( CPotentialPlayer *potential, unsigned char nPID, string nJoinedRealm, string nName, BYTEARRAY nInternalIP, bool nReserved ) : m_DBPlayer( NULL ), CPotentialPlayer( potential->m_Protocol, potential->m_Game, potential->GetSocket( ) ),
...
CGamePlayer :: CGamePlayer( CDBPlayer *nDBPlayer, CPotentialPlayer *potential, unsigned char nPID, string nJoinedRealm, string nName, BYTEARRAY nInternalIP, bool nReserved ) : m_DBPlayer( nDBPlayer ), CPotentialPlayer( potential->m_Protocol, potential->m_Game, potential->GetSocket( ) ),
m_PID( nPID ), m_Name( nName ), m_InternalIP( nInternalIP ), m_JoinedRealm( nJoinedRealm ), m_TotalPacketsSent( 0 ), m_TotalPacketsReceived( 1 ), m_LeftCode( PLAYERLEAVE_LOBBY ), m_LoginAttempts( 0 ), m_SyncCounter( 0 ), m_JoinTime( GetTime( ) ),
m_LastMapPartSent( 0 ), m_LastMapPartAcked( 0 ), m_StartedDownloadingTicks( 0 ), m_FinishedLoadingTicks( 0 ), m_StartedLaggingTicks( 0 ), m_StatsSentTime( 0 ), m_StatsDotASentTime( 0 ), m_LastGProxyWaitNoticeSentTime( 0 ), m_Score( -100000.0 ),
m_LoggedIn( false ), m_Spoofed( false ), m_SpoofedNoticeSent( false ), m_Reserved( nReserved ), m_WhoisShouldBeSent( false ), m_WhoisSent( false ), m_FingerSent(false), m_DownloadAllowed( false ), m_DownloadStarted( false ), m_DownloadFinished( false ), m_FinishedLoading( false ), m_Lagging( false ),
m_DropVote( false ), m_KickVote( false ), m_Muted( false ), m_LeftMessageSent( false ), m_GProxy( false ), m_GProxyDisconnectNoticeSent( false ), m_GProxyReconnectKey( GetTicks( ) ), m_LastGProxyAckTime( 0 ), m_XPAMGProxyVersion( 0 )
{
	// todotodo: properly copy queued packets to the new player, this just discards them
	// this isn't a big problem because official Warcraft III clients don't send any packets after the join request until they receive a response
	// m_Packets = potential->GetPackets( );
...
CDBDiv1DPS *CGamePlayer :: GetDiv1DPS( )
{
	if( m_DBPlayer )
		return m_DBPlayer->GetDiv1DPS( ) == NULL ? NULL : m_DBPlayer->GetDiv1DPS( );

	return NULL;
}

CDBGPS *CGamePlayer :: GetGPS( )
{
	if( m_DBPlayer )
		return m_DBPlayer->GetGPS( ) == NULL ? NULL : m_DBPlayer->GetGPS( );

	return NULL;
}

void CGamePlayer :: SetIgnorePID( unsigned char PID )
{
	for( unsigned int i = 0; i < m_IgnoredPIDs.size( ); ++i )
	{
		if( m_IgnoredPIDs[i] == PID )
			return;
	}

	m_IgnoredPIDs.push_back( PID );
}

bool CGamePlayer :: SetUnignorePID( unsigned char PID )
{
	for( BYTEARRAY :: iterator i = m_IgnoredPIDs.begin( ); i != m_IgnoredPIDs.end( ); ++i )
	{
		if( *i == PID )
		{
			m_IgnoredPIDs.erase( i );
			return true;
		}
	}
...
bool CGamePlayer :: GetIsBanned( )
{
	return m_DBPlayer == NULL ? false : m_DBPlayer->GetIsBanned( );
}

string CGamePlayer :: GetFrom( )
{
	return m_DBPlayer == NULL ? "??" : m_DBPlayer->GetFrom( );
}

void CGamePlayer :: SetFrom( string nFrom )
{
	if( m_DBPlayer )
		m_DBPlayer->SetFrom( nFrom );
}

string CGamePlayer :: GetLongFrom( )
{
	return m_DBPlayer == NULL ? "??" : m_DBPlayer->GetLongFrom( );
}

void CGamePlayer :: SetLongFrom( string nLongFrom )
{
	if( m_DBPlayer )
		m_DBPlayer->SetLongFrom( nLongFrom );
}

uint32_t CGamePlayer :: GetAccessLevel( )
{
	return m_DBPlayer == NULL ? 1 : m_DBPlayer->GetAccessLevel( );
}

bool CGamePlayer :: GetIsAdmin( )
{
	return m_DBPlayer == NULL ? false : m_DBPlayer->GetAccessLevel( ) > 1000;
}
...
bool CGamePlayer :: Update( void *fd )
{
	...
	if( m_WhoisShouldBeSent && !m_Spoofed && !m_WhoisSent && !m_JoinedRealm.empty( ) && GetTime( ) - m_JoinTime >= 4 )
	{
		...
		for( vector<CBNET *> :: iterator i = m_Game->m_GHost->m_BNETs.begin( ); i != m_Game->m_GHost->m_BNETs.end( ); ++i )
		{
			if( (*i)->GetServer( ) == m_JoinedRealm )
			{
				...
				if( (*i)->GetPasswordHashType( ) == "pvpgn" )
					(*i)->SendChatCommand( "/whereis " + m_Name );
			}
		}
	...
	// send /finger to check if player is muted on the server to apply a local mute
	if (m_Game->m_GHost->m_RequireMuteChecks && m_Spoofed && !m_FingerSent && !m_JoinedRealm.empty())
	{
		for (vector<CBNET *> ::iterator i = m_Game->m_GHost->m_BNETs.begin(); i != m_Game->m_GHost->m_BNETs.end(); ++i)
		{
			if ((*i)->GetServer() == m_JoinedRealm)
			{
				(*i)->SendChatCommand("/finger " + m_Name);
			}
		}
	...
	while( !m_Packets.empty( ) )
	{
		...
		if( Packet->GetPacketType( ) == W3GS_HEADER_CONSTANT )
		{
			...
			case CGameProtocol :: W3GS_MAPSIZE:
				if( m_Game->m_Map )
					MapSize = m_Protocol->RECEIVE_W3GS_MAPSIZE( Packet->GetData( ), m_Game->m_Map->GetMapSize( ) );
		...
		else if( Packet->GetPacketType( ) == GPS_HEADER_CONSTANT )
		{
			...
			if( Packet->GetID( ) == CGPSProtocol :: GPS_INIT )
			{
				...
				{
					// Get GProxy version
					m_XPAMGProxyVersion = UTIL_ByteArrayToUInt32(Data, false, 4);
					CONSOLE_Print( m_Name + " has GProxy version of " + UTIL_ToString( m_XPAMGProxyVersion ) );
				}
				else {
					CONSOLE_Print("[GHOST] Received a wrong data size for GPS_INIT");
					m_XPAMGProxyVersion = 0;
				}
			
				if( m_Game->m_GHost->m_Reconnect )
				{
					//Send request for w3 version only for ladder games if restricted mode is enabled
					if (m_Game->m_GHost->m_restrictW3Version) {// && m_Game->m_GHost->m_CurrentGame->GetGameType() == MASL_PROTOCOL::DB_DIV1_DOTA_GAME) {
						CONSOLE_Print("[GHOST] Sending GPSS_W3VERSION");
						m_Socket->PutBytes(m_Game->m_GHost->m_GPSProtocol->SEND_GPSS_W3VERSION(m_Game->m_GHost->m_restrictedW3Version));
					}
					else {
						//Send GPSS_INIT, regular path
						CONSOLE_Print("[GHOST] Sending GPSS_INIT");
						m_GProxy = true;
						...
						CONSOLE_Print("[GAME: " + m_Game->GetGameName() + "] player [" + m_Name + "] is using GProxy++ (regular mode)");
					}
				}
				else
				{
			...
			else if (Packet->GetID() == CGPSProtocol::GPS_W3VERSION) {
				if (Data.size() == 12) {
					uint16_t major = UTIL_ByteArrayToUInt16(Data, false, 0 + 4);
					uint16_t minor = UTIL_ByteArrayToUInt16(Data, false, 2 + 4);
					uint16_t patch = UTIL_ByteArrayToUInt16(Data, false, 4 + 4);
					uint16_t build = UTIL_ByteArrayToUInt16(Data, false, 6 + 4);
					W3Version w3vRemote(major, minor, patch, build);

					if (m_Game->m_GHost->m_restrictedW3Version->toString() != w3vRemote.toString()) {
						CONSOLE_Print("[GHOST] Remote sent invalid GPS_W3VERSION");
						CONSOLE_Print("[GHOST] Requested: "+ m_Game->m_GHost->m_restrictedW3Version->toString());
						CONSOLE_Print("[GHOST] Received: "+ w3vRemote.toString());

						//Kick
						m_Game->OpenSlot(GetPID(), true);
					}
					else {
						this->SetGpsW3VersionSent(true);
						CONSOLE_Print("[GHOST] Received correct GPS_W3VERSION");

						//Send GPSS_INIT, restricted path
						CONSOLE_Print("[GHOST] Sending GPSS_INIT");
						m_GProxy = true;
						m_Socket->PutBytes(m_Game->m_GHost->m_GPSProtocol->SEND_GPSS_INIT(m_Game->m_GHost->m_ReconnectPort, m_PID, m_GProxyReconnectKey, m_Game->GetGProxyEmptyActions()));
						CONSOLE_Print("[GAME: " + m_Game->GetGameName() + "] player [" + m_Name + "] is using GProxy++ (restricted mode)");
					}
				}
				else {
					CONSOLE_Print("[GHOST] Received a wrong data size for GPS_W3VERSION");
