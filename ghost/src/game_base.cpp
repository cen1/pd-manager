...
CGameLog :: CGameLog( CBaseGame *nGame ) : m_Game( nGame )
{
...
void CBaseGame :: StartCountDownAuto( bool requireSpoofChecks )
{
	if (!m_CountDownStarted)
	{
		...
		for (auto i = m_Reserved.begin(); i != m_Reserved.end(); ++i) {
			string pname = (*i);
			auto itFind = find_if(m_Players.begin(), m_Players.end(),
				[&pname](CGamePlayer* obj) {
		...
		for (auto s = m_Slots.begin(); s != m_Slots.end(); ++s) {
			CONSOLE_Print(UTIL_ToString(s->GetPID()) + " "
				+ UTIL_ToString(s->GetComputer()) + " "
				+ UTIL_ToString(s->GetSlotStatus()));
		}
		...
		if (!m_Reserved.empty() && m_Reserved.size()<=m_Slots.size())
		{
					...
					if (!reservedCopy.empty()) {
						...
						for (auto ss = m_Slots.begin(); ss != m_Slots.end(); ++ss) {
							if (ss->GetPID() == nextPlayer->GetPID()) {
								...
								CONSOLE_Print("Moving player " + nextPlayerName + " with PID " + UTIL_ToString(nextPlayer->GetPID()) 
									+ " to slot " + UTIL_ToString(slotCnt));
								matchFound = true;
							}
						}
			...
			CONSOLE_Print("WARN: Balance was not performed because size of reserved was "+UTIL_ToString(m_Reserved.size())
				+" and size of slots was "+UTIL_ToString(m_Slots.size()));
		}
	}
}
...
void CBaseGame::selectOneGameEventReporter() {
	...
	if (!this->m_Players.empty()) {
		...
		std::copy_if(this->m_Players.begin(), this->m_Players.end(), std::back_inserter(playersLeft),
			[](CGamePlayer* p) {return !p->GetDeleteMe(); });
