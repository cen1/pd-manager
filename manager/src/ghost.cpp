...
CGHost ::CGHost(CConfig* CFG, boost::asio::io_context& nIOService, boost::asio::io_context::work& nWork)
  : m_IOService(nIOService)
  , m_Work(nWork)
  , m_UpdateTimer(nIOService)
{
			...
			if ((*i)->m_DBGameID && !(*i)->m_Error) {
				...
				for (list<CDBPlayer2*>::iterator j = (*i)->m_DBPlayers2.begin(); j != (*i)->m_DBPlayers2.end(); ++j) {
					...
					if (NewDotAPlayerSummary) {
						// FIX: Use map instead of vector - no resize needed
						// Check if entry already exists and delete old one before replacing
						map<uint32_t, CDBDotAPlayerSummary*>::iterator it = m_DotAPlayerSummary.find((*j)->GetStatsDotAID());
						if (it != m_DotAPlayerSummary.end() && it->second != NULL)
							delete it->second;
