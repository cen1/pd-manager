...
class CCallableFromCheck : virtual public CBaseCallable {
  ...
  public:
	CCallableFromCheck(uint32_t nSlaveBotID, string nName, uint32_t nServerID, uint32_t nIP)
	  : CBaseCallable()
	  , m_SlaveBotID(nSlaveBotID)
	  , m_Name(nName)
...
class CCallableBanCheck : virtual public CBaseCallable {
  ...
  public:
	CCallableBanCheck(string nServer, string nUser)
	  : CBaseCallable()
	  , m_Server(nServer)
	  , m_User(nUser)
	  , m_Result(NULL)
...
class CCallableGameCustomAdd : virtual public CBaseCallable {
  ...
  public:
	CCallableGameCustomAdd(uint32_t nDBGameID, uint32_t nLobbyDuration, string nLobbyLog, string nGameLog, string nReplayName, string nMap, string nGameName, string nOwnerName, uint32_t nDuration, uint32_t nGameState, string nCreatorName, uint32_t nCreatorServerID, vector<CDBGamePlayer*> nDBGamePlayers, bool nRMK, uint32_t nGameType, std::string nRegion)
	  : CBaseCallable()
	  , m_Error(false)
	  , m_DBGameID(nDBGameID)
	  , m_DBCreatorPlayerID(0)
	  , m_LobbyDuration(nLobbyDuration)
	  , m_LobbyLog(nLobbyLog)
	  , m_GameLog(nGameLog)
	  , m_ReplayName(nReplayName)
	  , m_Map(nMap)
	  ...
	  , m_DBGamePlayers(nDBGamePlayers)
	  , m_RMK(nRMK)
...
class CCallableGameCustomDotAAdd : virtual public CBaseCallable {
  ...
  public:
	CCallableGameCustomDotAAdd(uint32_t nDBGameID, uint32_t nLobbyDuration, string nLobbyLog, string nGameLog, string nReplayName, string nMap, string nGameName, string nOwnerName, uint32_t nDuration, uint32_t nGameState, string nCreatorName, uint32_t nCreatorServerID, vector<CDBGamePlayer*> nDBGamePlayers, bool nRMK, uint32_t nGameType, uint32_t nWinner, uint32_t nCreepsSpawnedTime, uint32_t nCollectDotAStatsOverTime, uint32_t nMin, uint32_t nSec, vector<CDBDotAPlayer*> nDBDotAPlayers, string nMode1, string nMode2, std::string nRegion)
	  ...
	  , m_DBDotAGameID(0)
	  , m_Winner(nWinner)
	  , m_CreepsSpawnedTime(nCreepsSpawnedTime)
	  , m_CollectDotAStatsOverTime(nCollectDotAStatsOverTime)
	  , m_Min(nMin)
	  , m_Sec(nSec)
	  , m_DBDotAPlayers(nDBDotAPlayers)
	  , m_Mode1(nMode1)
	  , m_Mode2(nMode2)
	  , m_region(nRegion)
	{
...
class CCallableGameDiv1DotAAdd : virtual public CBaseCallable {
  ...
  public:
	CCallableGameDiv1DotAAdd(uint32_t nDBGameID, uint32_t nLobbyDuration, string nLobbyLog, string nGameLog, string nReplayName, string nMap, string nGameName, string nOwnerName, uint32_t nDuration, uint32_t nGameState, string nCreatorName, uint32_t nCreatorServerID, vector<CDBGamePlayer*> nDBGamePlayers, bool nRMK, uint32_t nGameType, uint32_t nWinner, uint32_t nCreepsSpawnedTime, uint32_t nCollectDotAStatsOverTime, uint32_t nMin, uint32_t nSec, vector<CDBDotAPlayer*> nDBDotAPlayers, string nMode1, string nMode2, bool nFF, bool nScored, vector<CDBDiv1DotAPlayer*> nDBDiv1DotAPlayers, std::string nRegion)
	  ...
	  , m_FF(nFF)
	  , m_Scored(nScored)
	  , m_DBDiv1DotAPlayers(nDBDiv1DotAPlayers)
	  , m_region(nRegion)
	{
	}
...
class CCallableBanAdd : virtual public CBaseCallable {
  ...
  public:
	CCallableBanAdd(string nPlayer, uint32_t nServerID, uint32_t nGameID)
	  : CBaseCallable()
	  , m_Player(nPlayer)
	  , m_ServerID(nServerID)
	  , m_GameID(nGameID)
...
class CCallableDIV1BanAdd : virtual public CBaseCallable {
  ...
  public:
	CCallableDIV1BanAdd(string nAdminPlayerName, uint32_t nAdminServerID, string nVictimPlayerName, uint32_t nVictimServerID, uint32_t nGameID, string nReason)
	  : CBaseCallable()
	  , m_AdminPlayerName(nAdminPlayerName)
	  , m_AdminServerID(nAdminServerID)
	  , m_VictimPlayerName(nVictimPlayerName)
	  , m_VictimServerID(nVictimServerID)
	  , m_GameID(nGameID)
	  , m_Reason(nReason)
	{
	}
...
class CCallableGameID : virtual public CBaseCallable {
  ...
  public:
	CCallableGameID(uint32_t nSlaveID, uint32_t nSlaveGameID, std::string nRegion)
	  : CBaseCallable()
	  , m_SlaveID(nSlaveID)
	  , m_SlaveGameID(nSlaveGameID)
	  , m_region(nRegion)
	  , m_Result(0)
	{
	}
...
class CCallableCacheList : virtual public CBaseCallable {
  ...
  public:
	CCallableCacheList(set<uint32_t> nServer)
	  : CBaseCallable()
	  , m_Server(nServer)
	{
	}
...
class CCallableContributorList : virtual public CBaseCallable {
  ...
  public:
	CCallableContributorList(uint32_t nServer)
	  : CBaseCallable()
	  , m_Server(nServer)
	{
	}
...
class CCallableDotAPlayerSummaryCheck : virtual public CBaseCallable {
  ...
  public:
	CCallableDotAPlayerSummaryCheck(string nName, string nServer)
	  : CBaseCallable()
	  , m_Name(nName)
	  , m_Server(nServer)
	  , m_Result(NULL)
...
class CDBCache {
  private:
	...
	map<uint32_t, CDBDotAPlayerSummary*> m_DPSList;

  public:
	CDBCache(
	  list<pair<uint32_t, std::unordered_map<string, CDBPlayer*>>> nPlayerList,
	  vector<CDBBanGroup*> nBanGroupList,
	  map<uint32_t, CDBDotAPlayerSummary*> nDPSList)
	  : m_PlayerList(nPlayerList)
	  , m_BanGroupList(nBanGroupList)
	  , m_DPSList(nDPSList)
	{
	}
	~CDBCache() {}
	...
	map<uint32_t, CDBDotAPlayerSummary*> GetDPSList() { return m_DPSList; }
};
...
class CDBPlayer {
  ...
  public:
	CDBPlayer(uint32_t nBanGroupID, uint32_t nStatsDotAID, uint32_t nAccessLevel)
	  : m_BanGroupID(nBanGroupID)
	  , m_StatsDotAID(nStatsDotAID)
...
class CDBPlayer2 {
  ...
  public:
	CDBPlayer2(bool nBanned, string nName, string nServer, uint32_t nColor, uint32_t nPlayerID, uint32_t nBanGroupID, uint32_t nStatsDotAID)
	  ...
	  , m_BanGroupID(nBanGroupID)
	  , m_StatsDotAID(nStatsDotAID)
	  , m_UpdateBanGroupID(false)
	  , m_UpdateStatsDotAID(false)
	  , m_BanGroup(NULL)
	  , m_DotAPlayerSummary(NULL)
	{
	}
	CDBPlayer2() {}
	~CDBPlayer2() {}
...
class CDBBanGroup {
  ...
  public:
	CDBBanGroup(uint32_t nDatetime, uint32_t nDuration, uint32_t nBanPoints, uint32_t nWarnPoints)
	  : m_Datetime(nDatetime)
	  , m_Duration(nDuration)
	  , m_BanPoints(nBanPoints)
	  , m_WarnPoints(nWarnPoints)
	{
	}
	~CDBBanGroup() {}
...
class CDBAutoBan {
  ...
  public:
	CDBAutoBan(uint32_t nPlayerID, uint32_t nDatetime, uint32_t nDuration, uint32_t nBanPoints)
	  : m_PlayerID(nPlayerID)
	  , m_Datetime(nDatetime)
	  , m_Duration(nDuration)
	  , m_BanPoints(nBanPoints)
...
class CDBDotAPlayerSummary {
  ...
  public:
	CDBDotAPlayerSummary(double nRating, double nHighestRating, uint32_t nTotalGames, uint32_t nTotalWins, uint32_t nTotalLosses, uint32_t nTotalKills, uint32_t nTotalDeaths, uint32_t nTotalCreepKills, uint32_t nTotalCreepDenies, uint32_t nTotalAssists, uint32_t nTotalNeutralKills, uint32_t nGamesObserved)
	  ...
	  , m_GamesObserved(nGamesObserved)
	{
	}
	~CDBDotAPlayerSummary() {}
