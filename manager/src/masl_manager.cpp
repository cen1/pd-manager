...
CPlayers ::CPlayers(CGHost* nGHost, string nServer, uint32_t nServerID)
  : m_GHost(nGHost)
  , m_Server(nServer)
...
CSlave ::CSlave(CGHost* nGHost, CManager* nManager, uint32_t nSlaveID, boost::asio::io_context& io_context)
  : socket_(io_context)
{
	m_GHost = nGHost;
	m_Manager = nManager;
								...
								if (ServerID == (*j)->GetServerID()) {
									...
									if (Player) {
										...
										if (DotAPlayerSummary) {
											...
											// FIX: Log error when DPS is NULL instead of silently defaulting to 1500
											CONSOLE_Print("[ERROR] Failed to get DPS for player [" + Name + "], StatsDotAID=" + UTIL_ToString(Player->GetStatsDotAID()) + ", defaulting to " + UTIL_ToString((uint32_t)MASL_PROTOCOL::DB_DIV1_STARTING_PSR) + " PSR");
										}
									}
...
CManager ::CManager(CGHost* nGHost, string nBindAddress, uint16_t nPort, string nGameListFile, uint32_t nGameListRefreshInterval)
  : m_GHost(nGHost)
  , m_BindAddress(nBindAddress)
  , m_Port(nPort)
  , m_GameListFile(nGameListFile)
  , m_GameListRefreshInterval(nGameListRefreshInterval)
{
...
CWebManager ::CWebManager(CGHost* nGHost, string nBindAddress, uint16_t nPort)
  : m_GHost(nGHost)
  , m_BindAddress(nBindAddress)
  , m_Port(nPort)
{
	...
	if (!Packet.empty()) {
		...
		switch (PacketID) {
			...
			case MASL_PROTOCOL ::WEB_UPDATE_DIV1_STATS_DOTA: {
				...
				// Check if entry already exists and delete old one before replacing
				map<uint32_t, CDBDotAPlayerSummary*>::iterator it = m_GHost->m_DotAPlayerSummary.find(DIV1StatsDotAID);
				if (it != m_GHost->m_DotAPlayerSummary.end() && it->second != NULL)
					delete it->second;

				// Insert/update the new DPS entry
