	...
	if (mysql_real_query((MYSQL*)conn, Query.c_str(), Query.size()) != 0)
		...
		if (Result) {
			...
			if (Row.size() == 2) {
				...
				DB_Log("[MYSQL] error checking from codes [" + UTIL_ToString(ip) + "] - row doesn't have 2 columns, it has " + UTIL_ToString(Row.size()));
			}
...
CDBCache* CacheList(void* conn, set<uint32_t> server)
{
	...
	map<uint32_t, CDBDotAPlayerSummary*> DPSList;
	...
	if (mysql_real_query((MYSQL*)conn, DPSQuery.c_str(), DPSQuery.size()) != 0)
		...
		if (Result) {
			...
			while (Row.size() == 13) {
				...
				// Insert directly into map using ID as key - no NULL entries needed
				DPSList[id] = new CDBDotAPlayerSummary(
				  UTIL_ToDouble(Row[1]),  // Rating
				  UTIL_ToDouble(Row[2]),  // Highest Rating
				  UTIL_ToUInt32(Row[3]),  // Games
				  UTIL_ToUInt32(Row[4]),  // Wins
...
{
	// FIX: Use map::find() to check for existence (not just bounds)
	map<uint32_t, CDBDotAPlayerSummary*>::iterator it = gGHost->m_DotAPlayerSummary.find(m_StatsDotAID);

	if (it != gGHost->m_DotAPlayerSummary.end()) {
		// Entry exists in map, return it
		return it->second;
	}

	// Entry not found in cache - log error for debugging
	CONSOLE_Print("[ERROR] DPS cache miss for StatsDotAID=" + UTIL_ToString(m_StatsDotAID) + " (entry does not exist in cache)");

	// TODO: Optional lazy loading from database could be added here
	// For now, return NULL and let caller handle the missing data
	return NULL;
}
