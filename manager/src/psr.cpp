...
void CPSR::CalculatePSR(vector<PairedPlayerRating>& team1, vector<PairedPlayerRating>& team2, uint32_t maxWinChanceDiffGainConstant)
{
	...
	for (unsigned int i = 0; i < m_team1.size(); ++i) {
		...
		CONSOLE_Print("[" + UTIL_ToString(i) + "] (" + UTIL_ToString(m_team1[i].second, 0) + ") " + UTIL_ToString(im_win, 0) + "/-" + UTIL_ToString(im_lose, 0));
	}
	...
	for (unsigned int i = 0; i < m_team2.size(); ++i) {
		...
		// Lose fix
		if (im_win == 1.0 && im_lose == 0.0)
			im_lose = 1.0;

		m_team2gainLose[i] = make_pair(im_win, im_lose);

		CONSOLE_Print("[" + UTIL_ToString(i) + "] (" + UTIL_ToString(m_team2[i].second, 0) + ") " + UTIL_ToString(im_win, 0) + "/-" + UTIL_ToString(im_lose, 0));
	}
